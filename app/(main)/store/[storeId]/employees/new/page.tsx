...
export default function EmployeeCreatePage() {
  ...
  const [form, setForm] = useState({
    ...
    name: '',
    phone: '',
    payPolicyId: null as number | null,
  });
    ...
    async function fetchPayPolicies() {
      try {
        ...
        const res = await fetch(
          `http://localhost:8080/api/stores/${storeId}/paypolicies`,
          {
            headers: {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:8080/api/employees/${storeId}`,
        {
          method: 'POST',
          headers: {
          ...
          <div>
            ...
            >
              ...
              {payPolicies.map((policy) => (
                ...
                [직급 {policy.jobTitle}] · 시급 {policy.hourlyWage.toLocaleString()}원
                </option>
              ))}
            </select>
          </div>
