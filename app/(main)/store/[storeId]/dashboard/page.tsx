...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        // 매장 정보
        const storeRes = await fetch(`http://3.37.87.159/api/stores/${storeId}`, {
        ...
        // 직원 요약
        const empRes = await fetch(`http://3.37.87.159/api/stores/${storeId}/employees/all`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        const schedRes = await fetch(`http://3.37.87.159/api/stores/${storeId}/schedules/date?workDate=${today}`, {
          headers: { Authorization: `Bearer ${token}` }, 
        });
