...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        // 매장 정보
        const storeRes = await fetch(`http://localhost:8080/api/stores/${storeId}`, {
        ...
        const schedRes = await fetch(`http://localhost:8080/api/stores/${storeId}/schedules/date?workDate=${today}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
